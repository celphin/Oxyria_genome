...
############################################
# Write code to run in 500bp ^2 parts
...
orthogroup_distances <- function(spp_hap, path=path, og_file=og_file, comb_file=comb_file, x_cpu=x_cpu){
  ...
  results <- parallel::parSapply(cl, base::seq_along(c(0:(round(length(orthogroup_list_total)/500)-1))), base::seq_along(c(0:(round(length(orthogroup_list_total)/500)-1))),
                                 orthogroup_loop, 
                                 spp_hap, path=path, orthogroups_in_spp=orthogroups_in_spp,
                                 orthogroup_list_total=orthogroup_list_total, 
                                 start_orthog=start_orthog, numb_orthogroups=numb_orthogroups)
}


orthogroup_loop <- function(i, j, spp_hap, path=path, orthogroups_in_spp=orthogroups_in_spp, orthogroup_list_total=orthogroup_list_total){
...
#################################
# Try joining Interproscan info (GOterms) to relevant genes

#cat *_interproscan_output.tsv >> Total_interproscan_output.tsv


# take GeneID, INTPRO, INTPRO des,  and GO columns
